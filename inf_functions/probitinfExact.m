% Given a GP prior belief (specified by the inputs) on probit(f), a set 
% of observations (x,probit(f(x))) and a setting of the GP hyperparameters, 
% this method provides a GPML-compatible way of computing the (negative 
% log) likelihood of the observations (x,f(x)) (not (x,probit(f(x)))) under 
...
% with additive Gaussian noise on the probit(f) observations. 
% 
...
function [post nlZ dnlZ] = probitinfExact(hyp,mn,cov,lik,x,y)
	if iscell(lik)
		likstr = lik{1};
	else
		likstr = lik;
	...
	mu = probitmean(cov,mn,hyp,x);     		% mean of the f-space GP at x
	K = probitcov(cov,mn,hyp,x);			% covariance of the f-space GP at x
	...
	alpha = solve_chol(L,normcdf(y)-mu);	% see GPML chapter 2.2 (http://gaussianprocess.org/gpml/chapters/RW2.pdf)
	...
	if nargout > 1
		nlZ = (normcdf(y)-mu)'*alpha/2+sum(log(diag(L)))+n*log(2*pi)/2;					% compute the negative log likelihood of the observations (x,y)
		if nargout > 2
			dnlZ = hyp;
			for i = 1:(m+D+1)
				dmu = probitmean(cov,mn,hyp,x,i);										% derivative of the f-space mean w.r.t. the i^(th) hyperparameter
				dK = probitcov(cov,mn,hyp,x,[],i);										% derivative of the f-space covariance w.r.t. the i^(th) hyperparameter
