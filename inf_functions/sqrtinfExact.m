% Given a GP prior belief (specified by the inputs) on sqrt(2f), a set 
% of observations (x,sqrt(2f(x))) and a setting of the GP hyperparameters, 
% this method provides a GPML-compatible way of computing the (negative 
% log) likelihood of the observations (x,f(x)) (not (x,sqrt(2f(x)))) under 
...
% with additive Gaussian noise on the sqrt(2f) observations. 
% 
...
function [post nlZ dnlZ] = sqrtinfExact(hyp,mn,cov,lik,x,y)
	if iscell(lik)
		likstr = lik{1};
	else
		likstr = lik;
	...
	mu = sqrtmean(cov,mn,hyp,x);     		% mean of the f-space GP at x
	K = sqrtcov(cov,mn,hyp,x);				% covariance of the f-space GP at x
	...
	alpha = solve_chol(L,0.5*y.^2-mu);		% see GPML chapter 2.2 (http://gaussianprocess.org/gpml/chapters/RW2.pdf)
	...
	if nargout > 1
		nlZ = (0.5*y.^2-mu)'*alpha/2+sum(log(diag(L)))+n*log(2*pi)/2; 					% compute the negative log likelihood of the observations (x,y)
		if nargout > 2
			dnlZ = hyp;
			for i = 1:(m+D+1)
				dmu = sqrtmean(cov,mn,hyp,x,i);											% derivative of the f-space mean w.r.t. the i^(th) hyperparameter
				dK = sqrtcov(cov,mn,hyp,x,[],i);										% derivative of the f-space covariance w.r.t. the i^(th) hyperparameter
